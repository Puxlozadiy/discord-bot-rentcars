...
async def messageHandling(bot, content, message_id, string, chooseCar, channel_id, log_channel, owner, view):  # принимает команды
    ...
    if (argument == 'вернули' or argument == 'свободен'):
        ...
        if (datetime.now().hour <= 24 and datetime.now().hour > 7):
            await message.edit(content=f"```{string} - Занят до рестарта {datetime.now().date().day + 1}.{datetime.now().date().month} ```", view=view[1])
